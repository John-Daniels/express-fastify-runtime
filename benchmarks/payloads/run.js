...
function generatePayload(sizeBytes) {
  const data = crypto
    .randomBytes(sizeBytes)
    .toString("hex")
    .substring(0, sizeBytes); // Approximate
  return { data };
}
...
async function runOne(name) {
  ...
  try {
    ...
    // Bench sizes
    await runAutocannon(port, "1KB", generatePayload(1024));
    // await runAutocannon(port, '10KB', generatePayload(10 * 1024)); // Skip intermediate for speed if needed
    // await runAutocannon(port, '100KB', generatePayload(100 * 1024));
